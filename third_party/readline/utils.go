...
const (
	...
	CharEscapeEx  = 91
	CharBackspace = 127
	// Internal synthetic key for Shift+Enter (e.g. kitty keyboard protocol: ESC [ 13 ; 2 u)
	CharShiftEnter = 1000
)
...
func escapeExKey(key *escapeKeyPair) rune {
	...
	case '~':
		...
		// Common Shift+Enter variants from terminals with modifyOtherKeys enabled.
		// - CSI 13;2~
		// - CSI 27;2;13~
		sp := strings.Split(key.attr, ";")
		if len(sp) == 2 {
			if code, err1 := strconv.Atoi(sp[0]); err1 == nil {
				if mod, err2 := strconv.Atoi(sp[1]); err2 == nil {
					if code == 13 && mod == 2 {
						r = CharShiftEnter
					}
				}
		...
		if len(sp) == 3 {
			if first, err1 := strconv.Atoi(sp[0]); err1 == nil {
				if mod, err2 := strconv.Atoi(sp[1]); err2 == nil {
					if code, err3 := strconv.Atoi(sp[2]); err3 == nil {
						if first == 27 && mod == 2 && code == 13 {
							r = CharShiftEnter
						}
					}
				}
